...
DATA_DIR = os.path.join(ROOT_DIR, 'data', 'UCI_HAR_Dataset')
OUT_DIR = os.path.join(ROOT_DIR, 'experiments', 'deepconvlstm', 'images')
...
def show_data_of_motions():
    ...
    :return: 曲线图
    """
    lm_path = os.path.join(DATA_DIR, "activity_labels.txt")
    ...
    out_path = os.path.join(OUT_DIR, "activity_lines.png")
...
def show_data_of_sensors():
    ...
    :return: 曲线图
    """
    paths, names = traverse_dir_files(os.path.join(ROOT_DIR, "data/UCI_HAR_Dataset/train/Inertial Signals"))
    label_data_dict = dict()
    for path, name in zip(paths, names):
        # 27表示坐下sit，249是行走walking，参考y_train.txt
    ...
    out_path = os.path.join(OUT_DIR, "sensors_lines.png")
    draw_dict(label_data_dict, color_list, out_path)
...
if __name__ == '__main__':
    show_data_of_motions()  # 区分运动信息
    show_data_of_sensors()  # 区分传感器信息
